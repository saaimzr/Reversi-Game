...
char* changeDirection(char boarddd[][26], int n, int row, int col, char colour,
...
void availMoves(const char board[][26], int rowMove[60], int colMove[60],
                int* totalMoves, char turn);
...
int minimaxSumMAX(char board[8][26], char origTurn, char current, int ply,
                  int alpha, int beta);

int minimaxSumMIN(char board[8][26], char origTurn, char current, int ply,
                  int alpha, int beta);
  ...
  for (int roww = 0; roww < n; roww++) {
    for (int coll = 0; coll < n; coll++) {
      if (roww == coll && (roww == n / 2 - 1 || roww == n / 2)) {
        board[roww][coll] = 'W';
      } else if ((roww == coll - 1 && roww == n / 2 - 1) ||
                 (roww == coll + 1 && roww == n / 2)) {
        board[roww][coll] = 'B';
      } else {
        board[roww][coll] = 'U';
      }
...
int weights(int i) {
  int weights[64] = {20, -3, 2,  2,  2,  2,  -3, 20, -3, -4, -1, -1, -1,
                     -1, -4, -3, 2,  -1, 1,  0,  0,  1,  -1, 2,  2,  -1,
                     0,  1,  1,  0,  -1, 2,  2,  -1, 0,  1,  1,  0,  -1,
                     2,  2,  -1, 1,  0,  0,  1,  -1, 2,  -3, -4, -1, -1,
                     -1, -1, -4, -3, 20, -3, 2,  2,  2,  2,  -3, 20};
  return weights[i];
}
